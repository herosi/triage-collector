 ...
 *
 * https://github.com/clymb3r/PowerShell/blob/master/Invoke-NinjaCopy/NTFSParser/NTFSParserDLL/NTFS_FileRecord.h
 *
...
public:
	...
	const CAttrBase* FindNextStream(_TCHAR *name = NULL, DWORD curnum = 0);
...
// Find Data attribute class of next record
const CAttrBase* CFileRecord::FindNextStream(_TCHAR *name, DWORD curnum)
{
	int i = 0;
	const CAttrBase *data = FindFirstAttr(ATTR_TYPE_DATA);
	while (data)
